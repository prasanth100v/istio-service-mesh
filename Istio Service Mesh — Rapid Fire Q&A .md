...
| 🏗️ Q7  | What proxy does Istio use?                 | 👉 Envoy ( high-performance, open-source proxy designed for `cloud-native applications` that handles all network traffic within the service mesh. |
...
| 🚦 Q25  | What is circuit breaking?                  | 👉 Prevents `cascading failures`. Where one failing service triggers a chain reaction of `failures across other services`.  |
